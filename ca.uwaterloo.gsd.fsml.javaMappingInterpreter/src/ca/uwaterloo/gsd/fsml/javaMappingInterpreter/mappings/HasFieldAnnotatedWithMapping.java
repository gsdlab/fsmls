...
public class HasFieldAnnotatedWithMapping extends JavaMapping {
	public HasFieldAnnotatedWithMapping(EObject element, EStructuralFeature feature, EAnnotation annotation, EClass concreteChildType, JavaMappingInterpreter interpreter, IProgressMonitor progressMonitor) throws FSMLMappingException {
		super(element, feature, annotation, concreteChildType, interpreter, progressMonitor);
	}
	public HasFieldAnnotatedWithMapping(EObject element, EStructuralFeature feature, EStructuralFeature essentialFeature, EAnnotation annotation, EClass concreteChildType, JavaMappingInterpreter interpreter, IProgressMonitor progressMonitor) throws FSMLMappingException {
		super(element, feature, essentialFeature, annotation, concreteChildType, interpreter, progressMonitor);
	}
	public HasFieldAnnotatedWithMapping(SyncItem syncItem, EAnnotation annotation, JavaMappingInterpreter interpreter, IProgressMonitor progressMonitor) throws FSMLMappingException {
		super(syncItem, annotation, interpreter, progressMonitor);
	}

	@Parameter(name=JavaMappingInterpreter.DETAIL_FULLY_QUALIFIED_TYPE, mode=Mode.REVERSE, required=true) 
	...
	protected boolean reverse() throws FSMLMappingException {
		...
		for (FieldDeclaration field : type.getFields()) {
			for (Object fieldModifier : field.modifiers()) {
				if (fieldModifier instanceof Annotation){
					String modifierType = ((Annotation)fieldModifier).getTypeName().getFullyQualifiedName(); 
					if (fullyQualifiedName.equalsIgnoreCase(modifierType)
						...
						foundAnnotation  = (Annotation) fieldModifier;
						break; 
					}
				}
			}
