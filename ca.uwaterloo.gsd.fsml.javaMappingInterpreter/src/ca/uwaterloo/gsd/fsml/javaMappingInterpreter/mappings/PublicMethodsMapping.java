...
public class PublicMethodsMapping extends JavaMapping {
	public PublicMethodsMapping(EObject element, EStructuralFeature feature, EAnnotation annotation, EClass concreteChildType, JavaMappingInterpreter interpreter, IProgressMonitor progressMonitor) throws FSMLMappingException {
		super(element, feature, annotation, concreteChildType, interpreter, progressMonitor);
	}
	public PublicMethodsMapping(EObject element, EStructuralFeature feature, EStructuralFeature essentialFeature, EAnnotation annotation, EClass concreteChildType, JavaMappingInterpreter interpreter, IProgressMonitor progressMonitor) throws FSMLMappingException {
		super(element, feature, essentialFeature, annotation, concreteChildType, interpreter, progressMonitor);
	}
	public PublicMethodsMapping(SyncItem syncItem, EAnnotation annotation, JavaMappingInterpreter interpreter, IProgressMonitor progressMonitor) throws FSMLMappingException {
	...
	protected boolean reverse() throws FSMLMappingException {
		...
		for (org.eclipse.jdt.core.dom.MethodDeclaration method : type.getMethods()) {
			...
			if (method.isConstructor() && annotation.getDetails().get("excludeConstructors")!= null){
				continue; 
			}
			for (Object modifier : method.modifiers()) {
				if (modifier instanceof Modifier){
					if ("public".equalsIgnoreCase(((Modifier)modifier).getKeyword().toString())){
						setFeatureContextAndMarker(method.getName().getFullyQualifiedName(), bodyDeclaration, method, null);
						break;
					}
				}
			}
